package it.communikein.myunimib.sync;
...
import it.communikein.myunimib.sync.enrolledexams.SyncUtilsEnrolled;
import it.communikein.myunimib.utilities.MyunimibDateUtils;
import it.communikein.myunimib.utilities.S3JsonUtils;
...
import static it.communikein.myunimib.sync.S3Helper.getHTML;
...
public class SyncTask {
    ...
    synchronized public static void syncBooklet(Context context) {
        ...
        try {
            ...
            ArrayList<BookletEntry> response = downloadBooklet(context);

            /* Parse the booklet into a list of content values */
            ContentValues[] bookletValues = S3JsonUtils.getBookletContentValues(response);
    ...
    synchronized public static void syncAvailableExams(Context context) {
        ...
        try {
            /* Retrieve the exams */
            ArrayList<AvailableExam> response = downloadAvailableExams(context);

            /* Parse the exams into a list of content values */
            ContentValues[] examsValues = S3JsonUtils.getAvailableExamsContentValues(response);
    ...
    synchronized public static void syncEnrolledExams(Context context) {
        ...
        try {
            /* Retrieve the exams */
            ArrayList<EnrolledExam> response = downloadEnrolledExams(context);

            /* Parse the exams into a list of content values */
            ContentValues[] examsValues = S3JsonUtils.getEnrolledExamsContentValues(response);
