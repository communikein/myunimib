...
import it.communikein.myunimib.utilities.PreferenceUtils;
...
public class SyncUtilsBooklet {
    ...
    synchronized private static void scheduleBookletSync(Context context) {
        if (sInitialized) return;

        Driver driver = new GooglePlayDriver(context);
        FirebaseJobDispatcher dispatcher = new FirebaseJobDispatcher(driver);
        Job constraintReminderJob = dispatcher.newJobBuilder()
                .setService(BookletJobService.class)
    ...
    private static void startImmediateSync(@NonNull final Context context) {
        Intent intentToSyncImmediately = new Intent(context, BookletSyncIntentService.class);
